...
def main(json_path='options/MIVF/MIVF_v1.json', checkpoints_path = 'experiments/MIVF/models/MIVF.pth'):
    ...
    parser.add_argument('--folder_lq', type=str, default='testsets/real_world_test', help='input low-quality test video folder')
    parser.add_argument('--folder_gt', type=str, help='input ground-truth test video folder')
    parser.add_argument('--checkpoints', type=str, default=checkpoints_path, help='checkpoints path')
    parser.add_argument('--tile', type=int, nargs='+', default=[4,240,240], help='Tile size, [0,0,0] for no tile during testing (testing as a whole)')
    parser.add_argument('--tile_overlap', type=int, nargs='+', default=[2,20,20], help='Overlapping of different tiles')
    ...
    task_name = opt['task']
    save_dir = f'results/{task_name}_{osp.basename(args.folder_lq)}'
