...
class ModelMambaVR(ModelPlain):
    """Train video restoration  with pixel loss"""
    def __init__(self, opt):
        super(ModelMambaVR, self).__init__(opt)
    ...
    def define_optimizer(self):
        ...
        if self.opt_train.get('fix_iter', 0) and len(self.fix_keys) > 0:
            ...
            if self.opt_train['G_optimizer_type'] == 'adam':
                ...
                self.G_optimizer = torch.optim.AdamW(G_optim_params, lr=self.opt_train['G_optimizer_lr'],
                            betas=self.opt_train['G_optimizer_betas'],
                            weight_decay=self.opt_train['G_optimizer_wd'])    
            ...
            super(ModelMambaVR, self).define_optimizer()
    ...
    def optimize_parameters(self, current_step):
        ...
        super(ModelMambaVR, self).optimize_parameters(current_step)
    ...
    def _test_video(self, lq):
        ...
        if num_frame_testing:
            ...
            # test as one clip (the whole video) if you have enough memory, 目前没有使用3DSwinT，因此不需要d_pad
    ...
    def _test_clip(self, lq):
        ...
        sf = self.opt['scale']
        window_size = self.opt['netG'].get('window_size', [6,8,8])
        # 确保window_size是列表或元组,目前没有使用3DSwinT，因此不需要元组的window_size
