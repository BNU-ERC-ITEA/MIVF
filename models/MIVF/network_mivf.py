...
from models.mambaIRv2.arch_util import to_2tuple, trunc_normal_
...
class ASSM(nn.Module):
    ...
    def forward(self, x, x_size, token):
        B, n, C = x.shape  # [B, D*H*W, C]
        H, W = x_size
        D = n // (H * W)

        full_embedding = self.embeddingB.weight @ token.weight  # [128, C]

        pred_route = self.route(x)  # [B, DHW, num_token]
        cls_policy = F.gumbel_softmax(pred_route, hard=True, dim=-1)  # [B, DHW, num_token]

        prompt = torch.matmul(cls_policy, full_embedding).view(B, n, self.d_state)

        detached_index = torch.argmax(cls_policy.detach(), dim=-1, keepdim=False).view(B, n)  # [B, DHW]
        x_sort_values, x_sort_indices = torch.sort(detached_index, dim=-1, stable=False)
        x_sort_indices_reverse = index_reverse(x_sort_indices)

        x = x.view(B, D, H * W, C)  # (B, D, H*W, C)
        x = x.view(B * D, H * W, C)  # (B*D, H*W, C)
        x = x.permute(0, 2, 1).reshape(B*D, C, H, W).contiguous()
        x = self.in_proj(x)
        x = x * torch.sigmoid(self.CPE(x))
        cc = x.shape[1]
        x = x.view(B * D, cc, H * W).permute(0, 2, 1).contiguous()  # (B*D, HW, cc)
        x = x.view(B, D * H * W, cc).contiguous()                    # (B, D*HW, cc)
...
class AttentiveLayer(nn.Module):
    def __init__(self,
        ...
        if self.num_frames > 1:
            self.temporal_pos_embed = nn.Parameter(torch.zeros(1, self.num_frames, dim))
            trunc_normal_(self.temporal_pos_embed, std=.02)
            self.tpe_alpha = nn.Parameter(torch.ones(1))
    ...
    def forward(self, x, frame_size, x_size, params):
        h, w = x_size
        b, n, c = x.shape
        c3 = 3 * c
        ...
        B_original = b // frame_size
        x = x.view(B_original, frame_size, n, c)  # (B, D, H*W, C)

        if self.num_frames > 1:
            x_t = x.permute(0, 2, 1, 3).contiguous().view(B_original * n, frame_size, c)  # (B*H*W, D, C)
            x_t = x_t + self.tpe_alpha * self.temporal_pos_embed
            x = x_t.view(B_original, n, frame_size, c).permute(0, 2, 1, 3).contiguous()  # (B, D, H*W, C)

        x = x.view(B_original, frame_size * n, c) # (B, D*H*W, C)
        x_normA = self.norm3(x)
        x_aca = self.assm(x_normA, x_size, self.embeddingA) + x
        x_aca = x_aca.view(B_original * frame_size, n, c)  # (B*D, H*W, C)
        x_normB = self.norm4(x_aca)
        x = x_aca + self.convffn2(x_normB, x_size)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, frame_size, x_size, params):
        ...
        for layer in self.layers:
            x = layer(x, frame_size, x_size, params)
...
class ASSB(nn.Module):
    ...
    def forward(self, x, x_size, params):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, self.frame_size, x_size, params), x_size))) + x
...
class MambaVR(nn.Module):
    ...
    def no_weight_decay(self):
        if self.num_frames > 1:
            return {'absolute_pos_embed', 'temporal_pos_embed'}
        else:
    ...
    def forward(self, x):
        B, D, C, h_ori, w_ori = x.shape
        assert D == self.num_frames, f"Input frames {D} != expected frames {self.num_frames}"
            
        x = x.view(B * D, C, h_ori, w_ori)
        ...
        x = x.view(B, D, C_out, H_out, W_out)
...
if __name__ == '__main__':
    ...
    # Test
    video_input = torch.randn([4, 4, 3, 64, 64]).cuda()
    video_output = model_video(video_input)
    print(f"Video Input: {video_input.shape} -> Output: {video_output.shape}")


    # Model Size
    total_img = sum([param.nelement() for param in model_video.parameters()])
    total_video = sum([param.nelement() for param in model_video.parameters()])
    print(f"\nVideo Model Parameters: {total_img / 1e6:.3f}M")
    print(f"Video Model Parameters: {total_video / 1e6:.3f}M")