...
class VSBMTestDataset(data.Dataset):
    """Video test dataset for recurrent architectures, which takes LR video
    frames as input and output corresponding HR video frames. Modified from
    https://github.com/xinntao/BasicSR/blob/master/basicsr/data/reds_dataset.py
    ...
    def __init__(self, opt):
        super(VSBMTestDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {'lq_path': [], 'gt_path': [], 'folder': [], 'idx': [], 'border': []}
    ...
    def __getitem__(self, index):
        ...
        if self.need_masks:
            # get gauss_mask
            pos_doc_name = f'{folder}.txt'
            pos_doc_path = os.path.join(self.pos_root, pos_doc_name)
            coords = np.loadtxt(pos_doc_path)
            for i in range(len(imgs_lq)):
                position = coords[i]
                image_shape = (imgs_lq[0].shape[1], imgs_lq[0].shape[2])
                ...
                sigma_1 = distance * 1.1
                sigma_2 = sigma_1 * imgs_lq[0].shape[1] / imgs_lq[0].shape[2]
                mask = utils_for_vflare.generate_anisotropic_gaussian_template(image_shape, sigma_1, sigma_2, angle)
                mask = np.stack([mask] * 3, axis=2)
                masks.append(mask)
        else:
            for i in range(len(imgs_lq)):
                mask = np.ones((imgs_lq[0].shape[1], imgs_lq[0].shape[2], imgs_lq[0].shape[0]), dtype=np.float32)
                masks.append(mask)

        masks = utils_video.img2tensor(masks)
...
class SingleVSBMTestDataset(data.Dataset):
    """Single video test dataset for recurrent architectures, which takes LR video
    frames as input and output corresponding HR video frames (only input LQ path).
    ...
    def __init__(self, opt):
        super(SingleVSBMTestDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.lq_root = opt['dataroot_lq']
        self.data_info = {'lq_path': [], 'folder': [], 'idx': [], 'border': []}
    ...
    def __getitem__(self, index):
        ...
        # imgs_lq_im = imgs_lq.permute(0, 2, 3, 1).numpy()
        # imgs_lq_im = (imgs_lq_im*255).astype(np.uint8)
        # for i in range(len(imgs_lq_im)):
        #     if i < len(imgs_lq_im)-5:
        #         mask = utils_for_vflare.flare_mask(imgs_lq_im[i],imgs_lq_im[i+1])
        #         masks.append(mask)
        #     else:
        #         mask = utils_for_vflare.flare_mask(imgs_lq_im[i],imgs_lq_im[i-6])
