...
TRAIN_SIZE = [432, 960] #used to be [432,960]
...
def prepare_image_for_vflare(viz_root_dir, fn1, fn2, keep_size):
    print(f"preparing image...")
    print(f"fn1 = {fn1}")
    print(f"fn1 = {fn2}")
...
def build_model_for_vflare():
    print(f"building  model...")
    cfg = get_cfg()
    model = torch.nn.DataParallel(build_flowformer(cfg))
    current_path = os.getcwd()
    print(current_path)
    full_path = os.path.join(current_path, "..", 'Flow generation\\FlowFormerPlusPlus\\checkpoints\\sintel.pth').replace("\\", "/")
    # full_path = os.path.join(current_path,  '..\\FlowFormerPlusPlus\\checkpoints\\sintel.pth').replace("\\", "/")  # path for dw linux
    # exit()
...
def save_flow(flow_save_dir, model, img_pairs):
    weights = None
    i = 0
    for img_pair in img_pairs:
        fn1, fn2 = img_pair
        print(f"processing {fn1}, {fn2}...")
        image1, image2 = prepare_image_for_vflare(flow_save_dir, fn1, fn2, True)
        flow = compute_flow(model, image1, image2, weights)
        np.save(os.path.join(flow_save_dir, f"{i:03}.png"), flow)
        i += 1
...
def generate_pairs(dirname, start_idx, end_idx):
    ...
    for idx in range(start_idx, end_idx):
        img1 = osp.join(dirname, f'{idx}.png')
        img2 = osp.join(dirname, f'{idx+1}.png')
        img_pairs.append((img1, img2))
    return img_pairs


def generate_pairs_for_vflare(dirname, start_idx, end_idx):
    img_pairs = []
    for idx in range(start_idx, end_idx):
        img1 = osp.join(dirname, f'{idx:03}.png').replace("\\", "/")
        img2 = osp.join(dirname, f'{idx+1:03}.png').replace("\\", "/")
        img_pairs.append((img1, img2))
    return img_pairs


def generate_flow(source_dir, target_dir, eval_type):
    soc_dir = source_dir
    pic_list = os.listdir(source_dir)
    len_pic_list = len(pic_list)
    flow_save_dir = target_dir
    model = build_model_for_vflare()
    if eval_type == 'sintel':
        img_pairs = process_sintel(args.sintel_dir)
    elif eval_type == 'seq':
        img_pairs = generate_pairs_for_vflare(soc_dir, 0, len_pic_list - 1)
    with torch.no_grad():
        save_flow(flow_save_dir, model, img_pairs)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--end_idx', type=int, default=7)    # ending index of the image sequence
    ...
    #     img_pairs = generate_pairs_for_vflare(args.seq_dir, args.start_idx, args.end_idx)
    # with torch.no_grad():
    ...
    img_pairs = generate_pairs_for_vflare(args.seq_dir, args.start_idx, args.end_idx)
    model = build_model()
    root_dir = r"E:\Video_Flare_Removal\res_paper\picInPaper\main_pipline\orgin"
    viz_root_dir = r"E:\Video_Flare_Removal\res_paper\picInPaper\main_pipline\flow"
    with torch.no_grad():
        visualize_flow(root_dir, viz_root_dir, model, img_pairs, True)