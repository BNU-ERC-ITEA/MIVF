...
struct LensDescription {
	...
	vector<PatentFormat> nikon_28_75mm = { // Lens prescription entries: {r (curvature radius), d (thickness/spacing), n (IOR), f (aperture stop), w/h (element size), c (coating/reflectance param)}
	...
	int num_of_ghosts = 352; // 27!/2*(27-2)! 352
...
struct Application {
	...
	float backbuffer_width = 1280;
	float backbuffer_height = 720;
...
// ---------------------------------------------------------------------------------------------------------
// Capture backbuffer and save rendered image
// ---------------------------------------------------------------------------------------------------------
void SaveRenderedImage() {
	// 1. Get the swapchain backbuffer
	ID3D11Texture2D* backBuffer;
	Win.d3d_swapchain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&backBuffer);

	// 2. Create a CPU-readable staging texture with the same format/size
	D3D11_TEXTURE2D_DESC desc;
	...
	Win.d3d_device->CreateTexture2D(&desc, nullptr, &stagingTexture);

	// 3. Copy backbuffer into the staging texture
	Win.d3d_context->CopyResource(stagingTexture, backBuffer);

	// 4. Map staging texture to CPU memory
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	Win.d3d_context->Map(stagingTexture, 0, D3D11_MAP_READ, 0, &mappedResource);
	// Get a pointer to the data in the texture
	unsigned char* data = reinterpret_cast<unsigned char*>(mappedResource.pData);

	// Set the alpha value for each pixel
	for (int i = 0; i < desc.Width * desc.Height * 4; i += 4) {
		data[i + 3] = 255; // Alpha channel
	}

	// 5. Save as PNG using stb_image_write
	stbi_write_png("output.png", desc.Width, desc.Height, 4, data, desc.Width * 4);

	// 6. Cleanup
	Win.d3d_context->Unmap(stagingTexture, 0);
	stagingTexture->Release();
	backBuffer->Release();
}

void SaveRenderedMultiImage(int image_index, std::string& filePath) {
	// 1. Get the swapchain backbuffer
	ID3D11Texture2D* backBuffer;
	Win.d3d_swapchain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&backBuffer);
	...
	// 5. Save as PNG using stb_image_write
	std::stringstream sfile_name;
	sfile_name << std::setw(3) << std::setfill('0') << image_index;
	std::string sub_path = filePath + "/" + sfile_name.str() + ".png";
	 
	char filename1[256];
	sprintf_s(filename1, sub_path.c_str());

	//char filename[256];
	//sprintf_s(filename, "%03d.png", image_index); // Ensure the index is zero-padded to 3 digits
	stbi_write_png(filename1, desc.Width, desc.Height, 4, data, desc.Width * 4);
...
std::vector<NormalizedCoord> ReadAndNormalizeCoords(const std::string& filePath, size_t& length) {
	std::ifstream file(filePath);
	if (!file.is_open()) {
		throw std::runtime_error("Failed to open file: " + filePath);
	}

	std::vector<NormalizedCoord> coords;
	std::string line;
	while (std::getline(file, line)) {
		...
		if (iss >> x >> y) {
			...
			float normalizedY = y / 1280.0f;
			coords.push_back({ normalizedX, normalizedY });
		}
	}
...
std::vector<std::vector<NormalizedCoord>> ReadAndNormalizeCoordsFromFolder(const std::string& folderPath) {
	std::vector<std::vector<NormalizedCoord>> allCoords;
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow) {
	...
	bool testflag = true; // Whether to run file-driven batch rendering

	// Input/output paths
	std::string filePath = "position_txt/lightsource_position_003.txt";
	std::string light_source_position_path = "E:/Video_Flare_Removal/data/sourcelight_temp";
	std::string reflect_flare_path = "E:/Video_Flare_Removal/data/Flare_mask/reflective_light/Reds";
	std::string previous_str;
	size_t coordsLength;
	// Read and normalize coordinates
	std::vector<NormalizedCoord> normalizedCoords = ReadAndNormalizeCoords(filePath, coordsLength);
	auto allCoords = ReadAndNormalizeCoordsFromFolder(light_source_position_path);
	int total_scene_num = allCoords.size();
	int total_pic_num_per_scene = allCoords[0].size();
	int save_pic_num = 0; // Number of images saved in the current scene
	int scene_num = 0; // Current scene index


	while (WM_QUIT != msg.message) {

		if (msg.message == WM_KEYDOWN) {
			// A: toggle wireframe overlay
			if (!UI.key_down && msg.wParam == 65)
				UI.overlay_wireframe = !UI.overlay_wireframe;
			// E: toggle coating-quality edit mode
			if (!UI.key_down && msg.wParam == 69)
				UI.editing_coating_quality = !UI.editing_coating_quality;
			// Q: start editing aperture opening
			if (!UI.key_down && msg.wParam == 81)
				UI.editing_aperture = true;
			// R: start editing number of aperture blades
			if (!UI.key_down && msg.wParam == 82)
				UI.editing_no_blades = true;
			// W: start editing ray spread
			if (!UI.key_down && msg.wParam == 87)
				UI.editing_spread = true;
			// Space: toggle 2D overlay
			if (!UI.key_down && msg.wParam == 32)
				UI.draw2d = !UI.draw2d;
			// Left arrow: decrease ghost bounce #1
			if (!UI.key_down && msg.wParam == 37)
				UI.ghost_bounce_1 = max<int>(2, UI.ghost_bounce_1 - 1);
			// Right arrow: increase ghost bounce #1
			if (!UI.key_down && msg.wParam == 39)
				UI.ghost_bounce_1 = min<int>((int)Lens.lens_interface.size() - 2, UI.ghost_bounce_1 + 1);
			// Down arrow: decrease ghost bounce #2
			if (!UI.key_down && msg.wParam == 40)
				UI.ghost_bounce_2 = max<int>(1, UI.ghost_bounce_2 - 1);
			// Up arrow: increase ghost bounce #2
			...
			// S: save current render (batch logic may auto-advance)
			if (!UI.key_down && msg.wParam == 83) //s
			{
				OutputDebugString(L"'S' key pressed\n");
				saveimage = true;
			}
		...
		if (msg.message == WM_MOUSEMOVE) {
			...
			float nx = ((p.x / App.backbuffer_width) * 1.f - 0.5f);
			float ny = ((p.y / App.backbuffer_height) * 0.5f - 0.25f);
			//std::ostringstream ss;

			//ss << "Aperture opening: " << UI.aperture_opening << endl
			//   << "Aperture blades: " << UI.number_of_blades << endl
			//   << "Coating quality: " << UI.coating_quality << endl
			//   << "Ray spread: " << UI.rays_spread << endl
			//   << "Mouse x: " << nx << endl
			//   << "Mouse y: " << ny << endl;
			...
			if (UI.editing_aperture) {
				UI.aperture_opening = 5.f + ny * 5.f * 4;
				UpdateLensComponents();
			}

			if (UI.editing_no_blades) {
				UI.number_of_blades = 10.f + ny * 4 * 5.f;
			}

			if (UI.editing_spread) {
				UI.rays_spread = 5.f + ny * 4 * 5.f;
			}

			if (UI.editing_coating_quality) {
				UI.coating_quality = 0.5f + ny * 16.0f;
			}
		...
		if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
			...
			if (testflag) {
				...
				std::uniform_real_distribution<float> distribution_rays(0.5f, 0.8f);
				std::uniform_real_distribution<float> distribution_color(1.00f, 1.20f);
				std::uniform_real_distribution<float> distribution_aperture_size(3.5f, 6.5f);
				if (scene_num < total_scene_num) {
					UI.number_of_blades = 8;
					UI.rays_spread = distribution_rays(generator);
					UI.coating_quality = distribution_color(generator);
					UI.aperture_opening = distribution_aperture_size(generator);
					UpdateLensComponents();
					UI.aperture_needs_updating = true;
					std::stringstream ss;
					ss << std::setw(4) << std::setfill('0') << scene_num;
					std::string sub_path = reflect_flare_path + "/" + ss.str();
					if (!std::filesystem::exists(sub_path)) {
						std::filesystem::create_directories(sub_path);
					}

					while (save_pic_num < total_pic_num_per_scene) {
						std::ostringstream ss;
						ss << "Scene: " << scene_num << "  Image: " << save_pic_num << endl
						   << "x: " << allCoords[scene_num][save_pic_num].x << "   y: " << allCoords[scene_num][save_pic_num].y << endl;
						OutputDebugStringA(ss.str().c_str());
						float sx = allCoords[scene_num][save_pic_num].y - 0.5f;
						float sy = (allCoords[scene_num][save_pic_num].x) * 0.5f - 0.25f;
						UI.x_dir = sx * 0.2f;
						UI.y_dir = sy * 0.2f;
						Render();
						SaveRenderedMultiImage(save_pic_num, sub_path);
						save_pic_num++;
					}
					scene_num++;
					save_pic_num = 0;
